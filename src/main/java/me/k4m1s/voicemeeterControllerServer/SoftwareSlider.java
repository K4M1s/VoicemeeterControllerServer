...
public class SoftwareSlider {
    ...
    public SoftwareSlider(int ID) {
        this.ID = ID + 3;

        this.trebleAddress = String.format("Strip[%d].EQGain3", this.ID);
        this.midAddress = String.format("Strip[%d].EQGain2", this.ID);
        this.bassAddress = String.format("Strip[%d].EQGain1", this.ID);
        ...
        this.monoAddress = String.format("Strip[%d].MC", this.ID);
    ...
    public void readData() {
        this.treble = Voicemeeter.getParameterFloat(this.trebleAddress);
        this.mid = Voicemeeter.getParameterFloat(this.midAddress);
        this.bass = Voicemeeter.getParameterFloat(this.bassAddress);
        this.gain = Voicemeeter.getParameterFloat(this.gainAddress);

        volumeLeft = Voicemeeter.getLevel(1, ((ID - 2) * 8) - 2);
        volumeRight = Voicemeeter.getLevel(1, ((ID - 2) * 8) - 1);
    ...
    public void setData(String data) {
        ...
        this.treble = Float.parseFloat(values[0]);
        this.mid = Float.parseFloat(values[1]);
        this.bass = Float.parseFloat(values[2]);
        this.gain = Float.parseFloat(values[3]);
        // 4 is volume value
        this.a1 = Boolean.parseBoolean(values[5]);
        this.a2 = Boolean.parseBoolean(values[6]);
        this.a3 = Boolean.parseBoolean(values[7]);
        this.b1 = Boolean.parseBoolean(values[8]);
        this.b2 = Boolean.parseBoolean(values[9]);
        this.mono = Boolean.parseBoolean(values[10]);
        this.solo = Boolean.parseBoolean(values[11]);
        this.mute = Boolean.parseBoolean(values[12]);
    }

    public void applyChanges() {
        Voicemeeter.setParameterFloat(this.bassAddress, this.bass);
        Voicemeeter.setParameterFloat(this.midAddress, this.mid);
        Voicemeeter.setParameterFloat(this.trebleAddress, this.treble);
    ...
    public String getLevels() {
        ...
        outLevel = String.format("VSS%d:%.1f,%.1f", ID - 3, lLevel, rLevel);
        return outLevel;
    }
    ...
    public String toString() {
        stripValueString = String.format("SS%d:%.2f,%.2f,%.2f,%.2f,%.2f,%b,%b,%b,%b,%b,%b,%b,%b", ID - 3, treble, mid, bass, gain, volume, a1, a2, a3, b1, b2, mono, solo, mute);
